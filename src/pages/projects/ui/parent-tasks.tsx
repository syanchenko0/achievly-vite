  ...
  return (
    <div className="bg-sidebar size-full max-h-full rounded-md border p-4">
      <div className="flex flex-col gap-y-4">
        <Link
          to={replacePathParams(ROUTES.project, { project_id: project.id })}
        >
        ...
        <span className="text-sm font-medium">Список родительских задач</span>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Accordion type="multiple">
              ...
              >
                ...
                <AccordionContent>
                  {!parent_task?.project_tasks?.length && (
                    <Alert>
                      <AlertTitle>Отсутствуют прикрепленные задачи</AlertTitle>
                      <AlertDescription>
                        Отредактируйте нужную вам задачу, указав родительскую
                        задачу
                      </AlertDescription>
                    </Alert>
                  )}
                      ...
                      <TableBody>
                        {parent_task?.project_tasks
                          ?.sort(
                            (a, b) =>
                              PROJECT_TASK_PRIORITY_WEIGHTS[
                                (b.priority as keyof typeof PROJECT_TASK_PRIORITY) ??
                                  "LOW"
                              ] -
                              PROJECT_TASK_PRIORITY_WEIGHTS[
                                (a.priority as keyof typeof PROJECT_TASK_PRIORITY) ??
                                  "LOW"
                              ],
                          )
                          ?.map((task) => (
